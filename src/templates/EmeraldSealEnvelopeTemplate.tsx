...
// Instead of the photographic floral corner art used by the gold template,
// this background is built from a hand-drawn gold filigree motif (arcs,
// a paisley flourish and scattered dots) rendered as inline SVG. That keeps
// the "royal seal" feeling intact while giving the template its own,
// image-free backdrop — pass `flipped` to reuse the same motif for the
// opposite corner via a 180° flip.
const OrnamentCorner: React.FC<{ flipped?: boolean; className?: string }> = ({ flipped, className }) => (
  <svg
    viewBox="0 0 300 300"
    className={`w-full h-full ${flipped ? 'scale-x-[-1] scale-y-[-1]' : ''} ${className || ''}`}
    fill="none"
  >
    {[60, 100, 140, 180, 220].map((r, i) => (
      <path
        key={r}
        d={`M 300,${300 - r} A ${r},${r} 0 0 1 ${300 - r},300`}
        stroke="currentColor"
        strokeWidth={i === 0 ? 1.4 : 0.6}
        opacity={0.55 - i * 0.08}
      />
    ))}
    <path d="M 300,115 A 185,185 0 0 1 115,300" stroke="currentColor" strokeWidth="0.6" strokeDasharray="2 7" opacity="0.55" />
    <path
      d="M 300,55 C 225,58 195,120 228,172 C 250,206 212,232 168,230 C 205,258 262,242 278,196 C 294,150 300,102 300,55 Z"
      fill="currentColor"
      opacity="0.12"
    />
    <path
      d="M 300,55 C 225,58 195,120 228,172 C 250,206 212,232 168,230"
      stroke="currentColor"
      strokeWidth="1"
      opacity="0.7"
    />
    <path d="M 300,20 C 260,20 260,60 300,60" stroke="currentColor" strokeWidth="0.8" opacity="0.5" />
    {[[262, 38], [227, 74], [192, 109], [157, 144], [122, 179]].map(([cx, cy], i) => (
      <circle key={i} cx={cx} cy={cy} r={2.4 - i * 0.3} fill="currentColor" opacity={0.6 - i * 0.09} />
    ))}
  </svg>
);

// Small flourish used in place of the gold template's floral badge/divider
// image — a simple laurel-leaf sprig with a central gem, drawn in gold.
const OrnamentDivider: React.FC<{ className?: string }> = ({ className }) => (
  <svg viewBox="0 0 200 60" className={className} fill="none">
    <path d="M100 30 C 80 10, 40 10, 20 30" stroke="currentColor" strokeWidth="1" opacity="0.7" />
    <path d="M100 30 C 120 10, 160 10, 180 30" stroke="currentColor" strokeWidth="1" opacity="0.7" />
    {[28, 44, 60].map((x, i) => (
      <ellipse key={`l-${x}`} cx={x} cy={22 - i * 2} rx="6" ry="3" fill="currentColor" opacity={0.55} transform={`rotate(-25 ${x} ${22 - i * 2})`} />
    ))}
    {[172, 156, 140].map((x, i) => (
      <ellipse key={`r-${x}`} cx={x} cy={22 - i * 2} rx="6" ry="3" fill="currentColor" opacity={0.55} transform={`rotate(25 ${x} ${22 - i * 2})`} />
    ))}
    <path d="M100 24 L106 30 L100 36 L94 30 Z" fill="currentColor" opacity="0.9" />
...
const CountdownDisplay: React.FC<{ weddingDateISO: string }> = ({ weddingDateISO }) => {
  ...
  return (
    <div className="grid grid-cols-4 gap-2 bg-[#0A3327]/90 backdrop-blur-md rounded-2xl p-4 border border-[#D4AF37]/40 shadow-inner text-center">
      {[
...
export const EmeraldSealEnvelopeTemplate: React.FC<TemplateRenderProps> = ({
  invitation,
  onOpenRSVP,
}) => {
  ...
  const OrnamentBackdrop = () => (
    <div className="fixed inset-0 pointer-events-none z-10 overflow-hidden text-[#D4AF37]">
      <motion.div
        initial={false}
        animate={{ y: [0, -6, 2, -4, 0], x: [0, 4, -3, 2, 0], rotate: [0, 2, -1.5, 1, 0], scale: [1, 1.015, 0.995, 1.01, 1] }}
        transition={{ duration: 7, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut' }}
        className="absolute -top-6 -right-6 w-72 h-72 sm:w-96 sm:h-96 md:w-[440px] md:h-[440px] opacity-80 origin-top-right"
      >
        <OrnamentCorner />
      </motion.div>
      <motion.div
        ...
        className="absolute -bottom-6 -left-6 w-72 h-72 sm:w-96 sm:h-96 md:w-[440px] md:h-[440px] opacity-80 origin-bottom-left"
      >
        <OrnamentCorner flipped />
      </motion.div>
  ...
  return (
    <div id="emerald-seal-template-root" className="relative">
      ...
      <AnimatePresence mode="wait">
        {!isOpened ? (
          ...
          >
            <div className="relative min-h-screen w-full bg-gradient-to-b from-[#062A1D] via-[#0B3D2E] to-[#041F15] text-[#F5EBD8] flex flex-col justify-between items-center overflow-hidden font-sans select-none">
              <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 text-[#D4AF37]">
                <motion.div
                  initial={false}
                  animate={{ y: [0, -6, 2, -4, 0], x: [0, 4, -3, 2, 0], rotate: [0, 2, -1.5, 1, 0], scale: [1, 1.015, 0.995, 1.01, 1] }}
                  transition={{ duration: 7, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut' }}
                  className="absolute -top-4 -right-4 w-72 h-72 sm:w-96 sm:h-96 md:w-[440px] md:h-[440px] pointer-events-none z-10 opacity-80 origin-top-right"
                >
                  <OrnamentCorner />
                </motion.div>
                <motion.div
                  ...
                  transition={{ duration: 8, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut' }}
                  className="absolute -bottom-4 -left-4 w-72 h-72 sm:w-96 sm:h-96 md:w-[440px] md:h-[440px] pointer-events-none z-10 opacity-80 origin-bottom-left"
                >
                  <OrnamentCorner flipped />
                </motion.div>
                {/* soft starlight dots scattered across the night-emerald backdrop */}
                {[
                  [8, 15], [22, 62], [12, 85], [88, 10], [92, 45], [78, 80], [55, 6], [45, 92], [65, 30], [32, 40],
                ].map(([left, top], i) => (
                  <span
                    key={i}
                    className="absolute rounded-full bg-[#F3E9D2] animate-pulse"
                    style={{
                      left: `${left}%`,
                      top: `${top}%`,
                      width: i % 3 === 0 ? '3px' : '2px',
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[550px] rounded-full bg-[#D4AF37]/10 blur-3xl pointer-events-none" />
              </div>
              ...
              <header className="relative z-30 w-full max-w-2xl px-6 pt-6 flex items-center justify-between">
                ...
                {isMusicModeOn && (
                  <button
                    ...
                    className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0B3D2E]/70 backdrop-blur-md border border-[#D4AF37]/40 shadow-sm hover:border-[#C5A059] transition-all text-[#F5EBD8]"
                  ...
                  >
                    {isMusicPlaying ? (
                      ...
                      <>
                        <VolumeX className="w-3.5 h-3.5 text-[#6E8F81]" />
                        <span className="text-[10px] font-semibold tracking-wider uppercase text-[#6E8F81]">Səs</span>
                      </>
                    )}
                  </button>
                )}
              ...
              <main className="relative z-20 my-auto w-full max-w-md px-6 py-4 flex flex-col items-center text-center">
                ...
                >
                  ...
                  >
                    <RibbonArt />
                    <h1 className="relative z-10 font-serif text-5xl sm:text-6xl font-light text-[#F3E9D2] tracking-widest uppercase leading-tight">
                      {groomName}
                    </h1>
                  ...
                  >
                    <div className="relative rounded-2xl bg-gradient-to-b from-white via-[#FCFAF7] to-[#F7F2EA] border-2 border-[#D4AF37]/50 shadow-[0_20px_50px_rgba(197,160,89,0.2)] p-6 flex flex-col items-center justify-center transition-all duration-300 group-hover:border-[#C5A059] group-hover:shadow-[0_25px_60px_rgba(197,160,89,0.35)]">
                      ...
                      <div className="mt-4">
                        <span className="text-xs font-bold tracking-[0.2em] text-[#0B3D2E] uppercase block">DƏVƏTNAMƏNİ AÇ</span>
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5 }} className="mt-4">
                    <p className="text-xs font-medium tracking-[0.2em] text-[#C9B98A] uppercase">
          ...
          <motion.div
            ...
            className="w-full min-h-screen"
          >
            <div className="w-full min-h-screen bg-[#F4FBF6] text-[#0B2E22] font-sans relative overflow-x-hidden selection:bg-[#C5A059] selection:text-white">
              <AnimatePresence>
                {toastMessage && (
                  <motion.div
                    ...
                    className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-[#0B2E22] text-[#F4FBF6] px-5 py-2.5 rounded-full text-xs font-medium shadow-2xl border border-[#C5A059]/50 flex items-center gap-2"
                  >
              ...
              <div className="fixed top-4 left-4 z-50">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white/95 border border-[#D4AF37]/40 shadow-md text-[#145C43] text-xs font-medium hover:border-[#C5A059] transition-all hover:scale-105"
                >
              ...
              <div className="min-h-screen w-full relative bg-[#F4FBF6] text-[#0B2E22] overflow-x-hidden">
                <OrnamentBackdrop />
                ...
                <div className="relative z-10 max-w-[500px] mx-auto min-h-screen px-4 pt-16 pb-28 flex flex-col items-center">
                  ...
                  <section className="w-full flex flex-col items-center text-center py-6 mb-4">
                    ...
                    >
                      ...
                      <h1 className="relative z-10 font-serif text-5xl sm:text-6xl md:text-7xl font-light text-[#0B2E22] tracking-[0.2em] uppercase leading-tight">
                        {groomName}
                      </h1>
                    ...
                    <motion.p
                      ...
                      className="text-xs sm:text-sm font-semibold tracking-[0.25em] text-[#5C8172] uppercase mt-4 mb-3"
                    >
                      {weddingDateFormatted} • {invitation.venue}
                    </motion.p>
                  ...
                  {invitation.heroImage && (
                    <section id="photo-section" className="w-full flex justify-center mb-10">
                      <motion.div
                        ...
                        className="relative rounded-3xl overflow-hidden bg-white p-2 border-2 border-[#D4AF37]/50 shadow-[0_20px_50px_rgba(11,46,34,0.18)] max-w-sm w-full group"
                      >
                  ...
                  <section id="countdown-section" className="w-full flex justify-center mb-10">
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      transition={{ duration: 0.8 }}
                      className="w-full max-w-sm sm:max-w-md bg-gradient-to-b from-[#062A1D] via-[#0C3A29] to-[#041F15] text-[#F5EBD8] rounded-3xl p-6 border-2 border-[#D4AF37]/50 shadow-[0_20px_50px_rgba(11,46,34,0.22)] relative overflow-hidden text-center"
                    >
                      <OrnamentDivider className="w-20 h-8 mx-auto mb-2 text-[#D4AF37] opacity-90" />
                  ...
                  <section id="program-section" className="w-full flex justify-center mb-10">
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      transition={{ duration: 0.8 }}
                      className="w-full max-w-sm sm:max-w-md bg-white/95 backdrop-blur-md rounded-3xl p-6 border-2 border-[#D4AF37]/50 shadow-[0_20px_50px_rgba(11,46,34,0.14)] text-center"
                    >
                      <OrnamentDivider className="w-24 h-9 mx-auto mb-1 text-[#D4AF37] opacity-85" />
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-6">
                        <div className="bg-[#F4FBF6] rounded-2xl p-3.5 text-center border border-[#D4AF37]/35 shadow-sm">
                          ...
                          <p className="font-serif text-base text-[#0B3D2E] font-semibold leading-tight">{weddingDateFormatted}</p>
                        </div>
                        <div className="bg-[#F4FBF6] rounded-2xl p-3.5 text-center border border-[#D4AF37]/35 shadow-sm">
                          ...
                          <p className="font-serif text-base text-[#0B3D2E] font-semibold leading-tight">{invitation.weddingTime}</p>
                        </div>
                      </div>
                        ...
                        <>
                          ...
                          <div className="space-y-2.5 text-left">
                            {invitation.schedule.map((item, idx) => (
                              <div key={idx} className="flex items-center gap-3.5 bg-[#F4FBF6] p-3 rounded-2xl border border-[#D4AF37]/30 shadow-sm">
                                ...
                                <div>
                                  <h4 className="font-serif text-base font-semibold text-[#0B3D2E] leading-tight">{item.title}</h4>
                                  {item.description && <p className="text-[10px] text-[#5C8172] mt-0.5">{item.description}</p>}
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      {invitation.dressCode && (
                        <div className="mt-5 inline-block px-4 py-2 rounded-xl bg-[#F4FBF6] border border-[#D4AF37]/30 text-xs text-[#145C43]">
                      ...
                      <div className="mt-6 flex justify-center">
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#145C43] hover:bg-[#C5A059] text-white text-xs font-semibold uppercase tracking-wider transition-all duration-300 shadow-md hover:scale-105"
                        >
                  ...
                  {invitation.customText && (
                    <section className="w-full flex justify-center mb-10">
                      ...
                      >
                        ...
                        <blockquote className="italic font-serif text-xl text-[#0B3D2E] leading-relaxed">“{invitation.customText}”</blockquote>
                  ...
                  <section id="venue-section" className="w-full flex justify-center mb-10">
                    <motion.div
                      ...
                      className="w-full max-w-sm sm:max-w-md bg-white/95 backdrop-blur-md rounded-3xl p-6 border-2 border-[#D4AF37]/50 shadow-[0_20px_50px_rgba(11,46,34,0.14)] text-center"
                    >
                      <OrnamentDivider className="w-24 h-9 mx-auto mb-1 text-[#D4AF37] opacity-80" />
                      <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[#C5A059] mb-1">Məkan Məlumatı</p>
                      <h2 className="font-serif text-3xl font-semibold text-[#0B2E22] mb-2">{invitation.venue}</h2>
                      <p className="text-xs text-[#4C6F60] max-w-xs mx-auto mb-6 leading-relaxed">{invitation.address}</p>
                      ...
                      <div className="flex flex-col gap-2.5">
                        <a
                          ...
                          className="w-full py-3.5 rounded-full bg-[#145C43] hover:bg-[#C5A059] text-white text-xs font-semibold uppercase tracking-wider shadow-md transition-all duration-300 flex items-center justify-center gap-2"
                        >
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full py-3.5 rounded-full bg-[#F4FBF6] border border-[#D4AF37]/40 text-[#145C43] hover:bg-white text-xs font-semibold uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2"
                        >
                  ...
                  <section id="rsvp-section" className="w-full flex justify-center mb-10">
                    ...
                    >
                      ...
                      <p className="italic font-serif text-base text-[#4C6F60] mb-6">Mərasimdə iştirak edəcəyinizi öncədən bildirməyinizi xahiş edirik.</p>
                      ...
                      <div className="flex flex-col gap-3 max-w-xs mx-auto">
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full py-3.5 px-6 rounded-full bg-[#C5A059] hover:bg-[#145C43] text-white text-xs font-semibold uppercase tracking-wider transition-all duration-300 shadow-md flex items-center justify-center gap-2 hover:scale-[1.02]"
                        >
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full py-3.5 px-6 rounded-full bg-[#F4FBF6] border border-[#D4AF37]/40 text-[#145C43] hover:border-[#145C43] text-xs font-semibold uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2 shadow-sm"
                        >
                  ...
                  <section className="w-full flex justify-center mb-10">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-white/95 hover:bg-[#C5A059] hover:text-white text-[#145C43] border-2 border-[#D4AF37]/50 text-xs font-semibold uppercase tracking-wider transition-all shadow-md hover:scale-105"
                    >
                  ...
                  <footer className="w-full py-8 text-center">
                    <div className="italic font-serif text-5xl text-[#145C43] mb-1">
                      {groomName.charAt(0)} & {brideName.charAt(0)}
                    </div>
                    <p className="text-[10px] tracking-[0.25em] text-[#3E6B5A] uppercase font-semibold">{weddingDateFormatted}</p>
                  </footer>
                </div>
              </div>
              ...
              <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 w-[95%] sm:w-auto max-w-[460px]">
                <div className="bg-[#062A1D]/98 border border-[#D4AF37]/40 rounded-full px-3.5 sm:px-5 py-2.5 shadow-2xl flex items-center justify-between sm:justify-around text-white gap-1 sm:gap-2">
                ...
                {showRsvpModal && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <motion.div
                      ...
                      className="bg-[#F4FBF6] border border-[#D4AF37]/40 rounded-2xl p-6 max-w-sm w-full shadow-2xl relative text-[#0B2E22]"
                    >
                      <button onClick={() => setShowRsvpModal(false)} className="absolute top-4 right-4 text-[#6E8F81] hover:text-[#0B2E22] text-lg">
                        ✕
                      </button>
                      <h3 className="font-serif text-2xl font-semibold text-[#145C43] text-center mb-1">İştirak Qeydiyyatı</h3>
                      <p className="text-[11px] text-[#6E8F81] text-center mb-4">
                        {groomName} & {brideName} — {weddingDateFormatted}
                      </p>
                      <form onSubmit={handleRsvpSubmit} className="space-y-3">
                        <div>
                          <label className="block text-[11px] font-semibold text-[#145C43] mb-1 uppercase tracking-wider">Adınız və Soyadınız</label>
                          <input
                            type="text"
                            required
                            ...
                            className="w-full bg-[#E7F3EC] border border-[#D4AF37]/30 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-[#C5A059]"
                          />
                        </div>
                        <div>
                          <label className="block text-[11px] font-semibold text-[#145C43] mb-1 uppercase tracking-wider">Gəliş Statusu</label>
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              type="button"
                              onClick={() => setRsvpAttending(true)}
                              className={`py-2 rounded-xl text-xs font-semibold border transition-all ${rsvpAttending === true ? 'bg-[#C5A059] text-white border-[#C5A059]' : 'bg-[#E7F3EC] text-[#145C43] border-[#D4AF37]/30'}`}
                            >
                              ✓ Qatılacağam
                            </button>
                            <button
                              ...
                              className={`py-2 rounded-xl text-xs font-semibold border transition-all ${rsvpAttending === false ? 'bg-[#145C43] text-white border-[#145C43]' : 'bg-[#E7F3EC] text-[#145C43] border-[#D4AF37]/30'}`}
                            >
                              ✕ Qatıla Bilmirəm
                            </button>
                          </div>
                        ...
                        {rsvpAttending && (
                          <div>
                            <label className="block text-[11px] font-semibold text-[#145C43] mb-1 uppercase tracking-wider">Qonaq Sayı</label>
                            <select
                              value={guestCount}
                        ...
                        <div>
                          <label className="block text-[11px] font-semibold text-[#145C43] mb-1 uppercase tracking-wider">Əlavə Qeyd (İstəyə görə)</label>
                          <textarea
                            rows={2}
                            placeholder="Məsələn: Təbriklər, sevinclə gələcəyik!"
                            value={guestNote}
                            onChange={(e) => setGuestNote(e.target.value)}
                            className="w-full bg-[#E7F3EC] border border-[#D4AF37]/30 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-[#C5A059] resize-none"
                          />
                        </div>
                        <button type="submit" className="w-full py-3 rounded-xl bg-[#C5A059] hover:bg-[#145C43] text-white font-semibold text-xs uppercase tracking-wider transition-colors shadow-md mt-2">
